...
def get_css(css):
    ...
    for rule in sheet:
        ...
        for i in range(len(styles)):
            new_tpl = tuple(styles[i].split(": "))
            styles[i] = new_tpl
        dct[selector] = dict(styles)
    return dct
...
def identify_tree(soup_parent):

    if soup_parent.name == "img":
        elem = ImgElement(soup_parent['src'])
        return elem
    else:
        elem = ComixElement(soup_parent.name)
        for child in soup_parent.children:
            if child == "\n":
                continue
            elem._content.append(identify_tree(child))
        return elem


def htmlToPython(html_file, css_file):
    ...
    project.set_backgrOutColor(css['html, body']['background-color'])
    project.set_backgrInColor(css['.container']['background-color'])
    project.set_width(css['.container']['max-width'])
    project.set_perspective(int(css['.parallax']['perspective'][:-2]))


    project._items.append(identify_tree(soup.body.div.div))
